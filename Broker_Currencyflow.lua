...
function Currencyflow:GetToday()
  ...
  local dst = date("*t").isdst and 3600 or 0
  return floor((time() + offset + dst) / 86400)
end
...
function Currencyflow:FormatGold(amt, colorize)
  ...
  elseif self.db.profile.cashFormat == 4 then
    -- With coin icons
    if gold > 0 then
      return sign .. format("|cff%s%s|r%s", color, BreakUpLargeNumbers(gold), ICON_GOLD)
    ...
    elseif silver > 0 then
      return sign .. format("|cff%s%d|r%s", color, silver, ICON_SILVER)
...
function Currencyflow:db_GetTotal(char, currency)
  local value = 0
  if char == 0 then
    for k, _ in pairs(self.db.realm.chars) do
      if self.db.realm.chars[k] and not self.db.realm.chars[k].ignore then
...
function Currencyflow:db_UpdateCurrency(currencyId, updateSession)
  local amount, earnedThisWeek, weeklyMax, totalMax
  ...
  if type(currencyId) == "number" then
    local lastWeekEarned = self.db.realm.chars[self.meidx]["lastWeekEarned" .. currencyId]
  ...
  if self.db.realm.chars[self.meidx].ignore then
    self.db.realm.chars[self.meidx].history = nil
    return
  end
  ...
  elseif tracking[currencyId].type == TYPE_CURRENCY then
    ...
    if weeklyMax and weeklyMax > 0 then
      self.db.realm.chars[self.meidx]["maxReached" .. currencyId] = earnedThisWeek >= weeklyMax / 100
      -- we can safely save the new earnedThisWeek value, since we checked for a reset before
      self.db.realm.chars[self.meidx]["lastWeekEarned" .. currencyId] = earnedThisWeek
    elseif totalMax and totalMax > 0 then
      self.db.realm.chars[self.meidx]["maxReached" .. currencyId] = amount >= totalMax / 100
    end
  elseif tracking[currencyId].type == TYPE_FRAGMENT then
    ...
    if amount then
      self.db.realm.chars[self.meidx]["maxReached" .. currencyId] = amount >= 200
    else
      amount = 0
    end
  ...
  if updateSession then
    if amount > oldVal then
      self.db.realm.chars[self.meidx].history[self.today][currencyId].gained = (self.db.realm.chars[self.meidx].history[self.today][currencyId].gained or 0) + amount - oldVal
...
function Currencyflow:addCharactersAndTotal()
  ...
  table.sort(self.db.realm.chars, function(a, b)
    if a == nil or b == nil then
      return false
    end

    -- Safety net for nil values.
    local a_value = nil == a[self.db.profile.sortChars] and 0 or a[self.db.profile.sortChars]
    local b_value = nil == b[self.db.profile.sortChars] and 0 or b[self.db.profile.sortChars]
  ...
  self.meidx = -1
  for k in pairs(self.db.realm.chars) do
    if self.db.realm.chars[k].charname == UnitName("player") then
      self.meidx = k
      break
    end
  end
  ...
  local function getLabelSegment(segment)
    ...
    elseif tracking[segment] then
      ...
      if tracking[segment].type == TYPE_CURRENCY or tracking[segment].type == TYPE_FRAGMENT then
        ...
        and self.db.realm.chars[self.meidx]["maxReached" .. segment] then
          color = COLOR_MAXREACHED
        else
          color = ""
        end
...
function Currencyflow:OptionsColumns()
  ...
  order = 201
  for k in pairs(currencies["current"]["pve"]) do
    addColumn(k)
  end
  ...
  order = 301
  for k in pairs(currencies["current"]["pvp"]) do
    addColumn(k)
  end
  ...
  order = 401
  for k in pairs(currencies["legacy"]["pve"]) do
    addColumn(k)
  end
  for k in pairs(currencies["legacy"]["pvp"]) do
    addColumn(k)
  end
  ...
  order = 601
  for k in pairs(currencies["archaeology"]) do
    addColumn(k)
  end
  ...
  order = 701
  for k in pairs(currencies["profession"]) do
    addColumn(k)
  end
  ...
  return {
    ...
    args = {
      ...
      ignoreChars = {
        ...
        values = function()
          local val = {}
          for k, v in pairs(self.db.realm.chars) do val[k] = v.charname end
          return val
        end,
        get = function(key, id) return self.db.realm.chars[id].ignore or false end,
        set = function(key, id, value) self.db.realm.chars[id].ignore = value end,
      },
      ...
      deleteChars = {
        ...
        values = function()
          local val = {}
          for k, v in pairs(self.db.realm.chars) do if k ~= self.meidx then val[k] = v.charname end end
          return val
        end,
      ...
      deleteConfirm = {
        ...
        func = function()
          if charToDelete ~= nil then self.db.realm.chars[charToDelete] = nil end
          charToDelete = nil
        end,
        width = "half",
        confirm = function()
          return format(L["CFG_CONFIRMDELETE"], self.db.realm.chars[charToDelete].charname)
        end,
...
function Currencyflow:LoadCurrencies()
  for id, currency in pairs(tracking) do
    ...
    elseif currency.type == TYPE_FRAGMENT then
      name, icon, _, _ = GetArchaeologyRaceInfo(currency.index)
...
function Currencyflow:OnEnable()
  ...
  if self.db.realm.chars then
    ...
    if cashflow.db.realm.version and cashflow.db.realm.version <= 9 then
      -- We can only copy the characters for the current faction/realm.
      Notice("Import database from Broker_Cashflow...")
      local realm_chars = deepcopy(cashflow.db.realm.chars)
      self.db.realm.version = cashflow.db.realm.version
      self.db.realm.chars = {}
      for _, v in pairs(realm_chars) do
        if v then
          table.insert(self.db.realm.chars, deepcopy(v))
        end
      end
      Notice("Import done.")
  ...
  if self.meidx == -1 then
    ...
    table.insert(self.db.realm.chars, {
      charname = UnitName("player"),
      class = classname,
      ignore = false,
      history = { [self.today] = { ["time"] = 0 } }
    })
    self.meidx = #self.db.realm.chars
  end
  ...
  for id in pairs(tracking) do self:db_UpdateCurrency(id, false) end
...
function Currencyflow:UpdateDatabase()
  ...
  if not self.db.realm.version then self.db.realm.version = 1 end

  -- Version 1 -> 2:
  --   Migrate characters table from realm.chars[name] to
  --   realm.chars[#].charname for easier sorting.
  --   Gotta love how "flexible" lua is!
  if self.db.realm.version == 1 then
    Notice("Updating database to version 2")
    local a = {}
    for k in pairs(self.db.realm.chars) do table.insert(a, k) end
    for _, k in pairs(a) do
      self.db.realm.chars[k].charname = k
      table.insert(self.db.realm.chars, self.db.realm.chars[k])
      self.db.realm.chars[k] = nil
    end
    self.db.realm.version = 2
    Notice("Update complete")
  end

  -- Version 2 -> 3:
  --   Move history to character level
  if self.db.realm.version == 2 then
    Notice("Updating database to version 3")
    for k in pairs(self.db.realm.chars) do
      self.db.realm.chars[k].history = { [self.today] = { ["time"] = 0 } }
    end
    self.db.realm.history = nil
    self.db.realm.version = 3
    Notice("Update complete")
  end

  -- Version 3 -> 4:
  --   Add "ignore" option to characters
  if self.db.realm.version == 3 then
    Notice("Updating database to version 4")
    for k in pairs(self.db.realm.chars) do
      self.db.realm.chars[k].ignore = false;
    end
    self.db.realm.version = 4
    Notice("Update complete")
  end

  -- Version 4 -> 5:
  --   Cataclysm!
  if self.db.realm.version == 4 then
    Notice("Updating database to version 5")
    for k in pairs(self.db.realm.chars) do
      -- Change ID of Champion's Seal (from item# 44990 to currency# 241)
      if self.db.realm.chars[k][44990] then
        self.db.realm.chars[k][241] = self.db.realm.chars[k][44990];
        self.db.realm.chars[k][44990] = nil;
      end

      -- Clean out the old stuff that doesn't exist anymore
      self.db.realm.chars[k][29434] = nil; -- Badge of Justice
      self.db.realm.chars[k][47241] = nil; -- Emblem of Triumph
      self.db.realm.chars[k][49426] = nil; -- Emblem of Frost
      self.db.realm.chars[k][29434] = nil; -- Badge of Justice
      self.db.realm.chars[k][20560] = nil; -- Alterac Valley MoH
      self.db.realm.chars[k][20559] = nil; -- Arathi Basin MoH
      self.db.realm.chars[k][29024] = nil; -- Eye of the Storm MoH
      self.db.realm.chars[k][47395] = nil; -- Isle of Conquest MoH
      self.db.realm.chars[k][42425] = nil; -- Strand o/t Anctients MoH
      self.db.realm.chars[k][20558] = nil; -- Warsong Gulch MoH
      self.db.realm.chars[k][43589] = nil; -- Wintergrasp MoH
      self.db.realm.chars[k][37836] = nil; -- Venture Coin
      self.db.realm.chars[k][43307] = nil; -- Stone Keeper's Shard
      self.db.realm.chars[k][43307] = nil; -- Old Arena Points
      self.db.realm.chars[k][43308] = nil; -- Old Honor Points
      ...
      self.db.realm.chars[k][392] = nil;
      self.db.realm.chars[k][395] = nil;
  ...
  if self.db.realm.version == 5 then
    ...
    for k in pairs(self.db.realm.chars) do
      ...
      if self.db.realm.chars[k][41596] then
        self.db.realm.chars[k][61] = self.db.realm.chars[k][41596];
        self.db.realm.chars[k][41596] = nil;
      end
      if self.db.realm.chars[k][43016] then
        self.db.realm.chars[k][81] = self.db.realm.chars[k][43016];
        self.db.realm.chars[k][43016] = nil;
      end
    end
    self.db.realm.version = 6
  end

  -- Version 6 -> 7:
  --   Introduced buttonFirst and buttonSecond config options
  if self.db.realm.version == 6 then
    Notice("Updating database to version 7")
    self.db.profile.buttonFirst = "2"
    self.db.profile.buttonSecond = "1"
    self.db.realm.version = 7
  end

  -- Version 7 -> 8:
  --   Introduced buttonThird and buttonFourth config options
  if self.db.realm.version == 7 then
    Notice("Updating database to version 8")
    self.db.profile.buttonThird = "1"
    self.db.profile.buttonFourth = "1"
    self.db.realm.version = 8
  end

  -- Version 8 -> 9:
  --   History indexes changed (got rid of faulty offset calculation)
  if self.db.realm.version == 8 then
  ...
  if self.db.realm.version == 9 then
...
function Currencyflow:RemoveOldData()
  ...
  for day in pairs(self.db.realm.chars[self.meidx].history) do
    if day < lastMonth then self.db.realm.chars[self.meidx].history[day] = nil end
  end
...
function Currencyflow:UpdateCurrencies() -- Update all currencies
  for id in pairs(tracking) do self:db_UpdateCurrency(id, true) end
  self:UpdateLabel()
end
